...
"""
Two-stage LightGBM v2 - C_two_stage focused with feature importance & auto-selection.

Changes from v1:
- Removed A_no_news and B_news_direct (focus on C_two_stage only)
- Added feature importance tracking across folds
- Auto feature selection: importance-based top-K filtering after warmup fold
- Stage2 fine-tune uses lower LR (0.01) to preserve Stage1 learning
- Enhanced news training diagnostics
- Embargo gap between train/test to prevent target leakage
...
# Stage2: lower LR for fine-tuning, fewer trees
LGB_PARAMS_STAGE2 = dict(
    objective="regression",
    boosting_type="gbdt",
    max_depth=4,          # shallower to avoid overfitting on small news subset
    num_leaves=15,
    learning_rate=0.01,   # 5x lower than Stage1
    n_estimators=300,     # fewer iterations for fine-tune
    min_child_samples=20, # stricter to prevent overfitting on sparse news data
    min_split_gain=1e-3,
    subsample=0.8,
    subsample_freq=1,
...
FEATURE_SELECTION = dict(
    warmup_folds=1,       # folds before activating auto-selection
    min_features=15,      # minimum features to keep
    importance_pct=0.85,  # keep features covering this % of cumulative importance
)
...
class FeatureTracker:
    """Accumulates feature importance across folds for auto-selection."""
    ...
    def update(self, model: lgb.LGBMRegressor, feature_names: List[str],
               fold_id: int, horizon: int, stage: str):
    ...
    def summary(self, all_features: List[str]) -> pd.DataFrame:
        """Return aggregated importance per feature, sorted descending."""
        rows = []
...
def make_folds(
    ...
    """
    Expanding walk-forward with optional embargo gap.
    embargo_days: number of trading days to skip between train end and test start
                  to prevent target leakage from forward returns.
    """
    ...
    fold_id = 1
    while True:
        # embargo gap: skip embargo_days after train end
...
def train_fold_c_two_stage(
    ...
    """
    Train C_two_stage only, with diagnostics and importance tracking.

    Stage1: train on all data with stage2 schema (news cols zeroed)
    Stage2: fine-tune on news-only rows with actual news features, lower LR
    """
    ...
    # ── Stage1: zero out news cols, train on full data ──
    train_s1 = train_part.copy()
    valid_s1 = valid_part.copy()
    test_s1 = fold_test.copy()
    for c in NEWS_COLS + ["has_news"]:
        if c in train_s1.columns:
            train_s1[c] = 0.0
            valid_s1[c] = 0.0
            test_s1[c] = 0.0

    m_stage1 = fit_lgbm(
        train_s1[stage2_feats], train_s1[target_col],
        valid_s1[stage2_feats], valid_s1[target_col],
        params=LGB_PARAMS_STAGE1,
    )
    base_pred = m_stage1.predict(test_s1[stage2_feats], num_iteration=m_stage1.best_iteration_)

    # Track Stage1 importance
    tracker.update(m_stage1, stage2_feats, fold_id, horizon, "stage1")

    # ── Stage2: fine-tune on news rows with lower LR ──
    news_train = train_part["news_count"] > 0
    news_valid = valid_part["news_count"] > 0
    news_test = fold_test["news_count"] > 0

    n_news_tr = int(news_train.sum())
    n_news_va = int(news_valid.sum())
    n_news_te = int(news_test.sum())
    print(
        f"  [Stage2 diag] news rows - train: {n_news_tr}, valid: {n_news_va}, "
        f"test: {n_news_te} / total test: {len(fold_test)}"
    )

    pred_c = base_pred.copy()
    min_rows = 50

    if n_news_tr >= min_rows and n_news_va >= max(10, min_rows // 5):
        m_stage2 = fit_lgbm(
            train_part.loc[news_train, stage2_feats],
            train_part.loc[news_train, target_col],
            valid_part.loc[news_valid, stage2_feats],
            valid_part.loc[news_valid, target_col],
            params=LGB_PARAMS_STAGE2,       # lower LR
            init_model=m_stage1.booster_,
        )

        # Track Stage2 importance
        tracker.update(m_stage2, stage2_feats, fold_id, horizon, "stage2")

        if news_test.any():
            pred_news = m_stage2.predict(
                fold_test.loc[news_test, stage2_feats],
                num_iteration=m_stage2.best_iteration_,
            )
            pred_c[np.where(news_test.values)[0]] = pred_news
            print(f"  [Stage2] applied to {n_news_te} test rows (best_iter={m_stage2.best_iteration_})")
        else:
            print("  [Stage2] no news rows in test - using Stage1 predictions only")
    else:
        reason = []
        if n_news_tr < min_rows:
            reason.append(f"train news rows ({n_news_tr}) < {min_rows}")
        if n_news_va < max(10, min_rows // 5):
            reason.append(f"valid news rows ({n_news_va}) < {max(10, min_rows // 5)}")
        print(f"  [Stage2] SKIPPED - {', '.join(reason)}")

    return pred_c


# ─── Backtest (unchanged from v1) ───────────────────────────────────────────
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Two-stage LightGBM v2 (C_two_stage focused)")
    ...
    ap.add_argument("--embargo-days", type=int, default=0,
                    help="Trading days to skip between train end and test start (default: 0, "
                         "set to max(horizons) e.g. 20 for strict leakage prevention)")
    ap.add_argument("--auto-select", action="store_true", default=True,
                    help="Enable auto feature selection after warmup folds")
    ...
    print(f"=== Two-stage LightGBM v2 (C_two_stage focused) ===")
    ...
    print(f"Stage1 features (no news): {len(stage1_feats)}")
    print(f"Stage2 features (with news): {len(stage2_feats)}")
    print(f"News columns: {NEWS_COLS}")
    print(f"Embargo gap: {args.embargo_days} trading days")
    print(f"Auto feature selection: {args.auto_select}")
    print(f"[TIME] data prep: {t1_load - t0_load:.2f}s")

    # News coverage diagnostics
    news_mask = df["news_count"] > 0
    print(f"\n--- News Coverage ---")
    print(f"Total rows with news: {news_mask.sum():,} / {len(df):,} ({news_mask.mean()*100:.1f}%)")
    if news_mask.any():
        news_dates = df.loc[news_mask, "date"]
        print(f"News date range: {news_dates.min().date()} ~ {news_dates.max().date()}")
        print(f"Unique stocks with news: {df.loc[news_mask, 'stock_code'].nunique()}")
    print()
    ...
    for f in folds:
        print(f"  Fold {f.fold_id}: train <= {f.train_end_date.date()} | "
    ...
    for horizon in horizons:
        ...
        print(f"\n{'='*60}")
        print(f"Horizon: {horizon} days (target: {target_col})")
        print(f"{'='*60}")
        ...
        mask_clean = ds[stage2_feats + [target_col]].notna().all(axis=1)
        print(f"Rows removed (NA): {int((~mask_clean).sum()):,}")

        ds_clean = ds.loc[mask_clean].copy().reset_index(drop=True)
        if len(ds_clean) == 0:
            print(f"Skipped: no rows after NA drop")
            continue

        # Determine which features to use (may be filtered after warmup)
        active_stage2 = stage2_feats.copy()
        ...
        for fold in folds:
            ...
            # Auto feature selection after warmup folds
            if (args.auto_select
                ...
                if len(selected) < len(stage2_feats):
                    dropped = len(stage2_feats) - len(selected)
                    print(f"[F{fold.fold_id}] Auto-select: {len(selected)} features "
                          f"(dropped {dropped} low-importance)")
                active_stage2 = selected
            else:
                active_stage2 = stage2_feats.copy()

            print(f"\n[H{horizon}][F{fold.fold_id}] train={len(tr):,} test={len(te):,} "
                  f"feats={len(active_stage2)}")

            y_pred = train_fold_c_two_stage(
                tr, te, target_col,
                stage1_feats, active_stage2,
                tracker, fold.fold_id, horizon,
            )
    ...
    print(f"\n[TIME] model train/eval total: {t1_train - t0_train:.2f}s")

    if not all_fold_metrics or not all_preds:
        raise SystemExit("No predictions produced.")

    # ── Feature importance report ──
    imp_summary = tracker.summary(stage2_feats)
    if len(imp_summary) > 0:
        print(f"\n{'='*60}")
        print("Feature Importance (Top 30, avg across folds)")
        print(f"{'='*60}")
        ...
        print(f"\nAuto-selected features ({len(selected_final)}/{len(stage2_feats)}):")
        for i, f in enumerate(selected_final, 1):
            avg = tracker.importance_sum.get(f, 0) / max(1, tracker.importance_count.get(f, 1))
            is_news = "[NEWS]" if f in NEWS_COLS or f == "has_news" else "      "
            print(f"  {i:3d}. {is_news} {f:<40s} avg_imp={avg:.1f}")
        ...
        if len(imp_detail) > 0:
            ...
            print(f"\nFeature importance detail saved: {args.out_importance}")
    ...
    out_summary = {
        "config": {
            ...
            "lgb_params_stage1": LGB_PARAMS_STAGE1,
            "lgb_params_stage2": LGB_PARAMS_STAGE2,
